...
my $fq2 = $ARGV[1];
my $i1 = $ARGV[2];
my $barcode_file = $ARGV[3];

unless (defined $ARGV[0] and defined $ARGV[1] and defined $ARGV[2] and defined $ARGV[3]){
  say "$0 file.R1.fastq file.R2.fastq i1.fastq barcode.txt";
  say "\nBarcode file (Tab separated):\nSample1 CTTGTA\nSample2 ATCACG";
  exit;
}
...
open( UNKNOWN_R1, "| gzip -1 > unknown-barcode_$fq1-$barcode_file.fq.gz" )
  or die "cannot create unknown-barcode_$fq1.fq";
open( UNKNOWN_R2, "| gzip -1 > unknown-barcode_$fq2-$barcode_file.fq.gz" )
  or die "cannot create unknown-barcode_$fq2.fq";
...
while ( my $p1_first_line  = <INFQ1>, my $p1_second_line = <INFQ1>, my $p1_third_line  = <INFQ1>, my $p1_fourth_line = <INFQ1>,
	...
	chomp( $p1_first_line, $p1_second_line, $p1_third_line, $p1_fourth_line, $p2_first_line, $p2_second_line, $p2_third_line, $p2_fourth_line, $i1_second_line);
...
# no match count to avoid duplication inside loop: to compare with total barcodes, if equal, then no match
	my $nomatch_count = 0;
	for ( my $j = 0 ; $j < scalar @barcode ; $j++ ) {
		
	  # get 6 bp
	  my $sixBp_i1_second_line = substr($i1_second_line, 0, 6);
		if ( $sixBp_i1_second_line eq $barcode[$j] ) {
